        ...
        <div class="sm:py-32 sm:relative">
            <div>
                <h2 class="text-3xl font-extrabold sm:text-5xl">
                    {{ $saleCollection->translateAttribute('name') }}
                </h2>

                @if ($saleCollection->translateAttribute('description'))
                    <p class="mt-1 text-lg font-medium">
                        {{ $saleCollection->translateAttribute('description') }}
                    </p>
                @endif

                <a href="{{ route('collection.view', $saleCollection->defaultUrl->slug) }}"
            ...
            <div class="mt-8 sm:absolute sm:right-0 sm:top-0 sm:mt-0">
                <div class="flex flex-col">
                    @foreach ($saleCollectionImages as $imageGroup)
